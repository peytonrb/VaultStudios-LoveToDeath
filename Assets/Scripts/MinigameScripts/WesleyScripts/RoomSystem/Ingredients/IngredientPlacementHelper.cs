...
public class IngredientPlacementHelper
{
    ...
    public IngredientPlacementHelper(HashSet<Vector2Int> roomFloor, 
        HashSet<Vector2Int> roomFloorNoCorridor)
    {
    ...
    public Vector2? GetIngredientPlacementPosition(PlacementType placementType, int iterationsMax, Vector2Int size, bool addOffset)
    {
        int ingredientArea = size.x * size.y;
        if (tileByType[placementType].Count < ingredientArea)
            return null;
        ...
        while (iteration < iterationsMax)
        {
            ...
            if (ingredientArea > 1)
            {
                var (result, placementPositions) = PlaceBigIngredient(position, size, addOffset);
    ...
    private (bool, List<Vector2Int>) PlaceBigIngredient(
